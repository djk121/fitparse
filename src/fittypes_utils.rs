use fitparsers::{parse_uint32, parse_date_time};
    ...
    ($s:ident, $f:ident) => {
        if $s.unknown_fields.len() > 0 {
            for (field_number, field_value) in $s.unknown_fields.iter() {
                writeln!($f, "  {: >28}: {}", format!("unknown_{}", field_number), field_value)?;
...
impl FitFieldLocalDateTime {
    pub fn parse(input: &[u8], endianness: Endianness, _offset_secs: f64) -> Result<FitFieldLocalDateTime> {
