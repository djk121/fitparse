...
use {
    ...
    FitNormalRecordHeaderMessageType, FitDeveloperFieldDefinition, FitFileHeader
};
...
pub fn parse_enum(input: &[u8], parse_config: &FitParseConfig) -> Result<u8> {
    let (_, res) = parse_enum_inner(input, parse_config).map_err(|e| errors::nom_parsing_error(e.to_string()))?;
...
pub fn parse_enum_inner<'a>(input: &'a [u8], parse_config: &FitParseConfig) -> nom::IResult<&'a [u8], &'a [u8]> {
    let (i, res) = nom::bytes::complete::take(1usize)(input)?;
    Ok((i, res))
}
...
/*
named!(parse_normal_record_header2<&[u8], FitRecordHeader>,
    do_parse!(
        first_byte:
            bits!(
                tuple!(
                    tag_bits!(1, 0x0), // should be 0 for Normal
                    take_bits!(1), // data or definition flag
                    take_bits!(1), // developer data flag
                    tag_bits!(1, 0x0), // reserved
                    take_bits!(4) // local message type
            )) >>
...
pub fn parse_normal_record_header(i: &[u8]) -> nom::IResult<&[u8], FitRecordHeader> {
    ...
    let p1 = nom::bits::complete::tag(0x0, 1_usize);
    let (i, _): (&[u8], u8) = nom::bits::bits(p1)(i)?;

    //let (i, ) = nom::bits::bits(nom::bits::complete::tag(0x0, 1_usize))(i);

    //let (i, _) = nom::bits::bits(nom::bits::complete::tag(0x0, 1_usize))(i)?;

    //let (i, _): (&[u8], u8) = nom::bits::bits(nom::bits::complete::tag(0x0, 1_usize))(i)?;       // should be 0 for Normal

    //let (i, _) = nom::bits::bits(nom::bits::complete::tag::<&[u8], (&[u8], u8), usize, dyn nom::error::ParseError<(&[u8], usize)>>(0, 1_usize))(i)?;

    //let (i, _) = nom::bits::bits<&[u8], (&[u8], _), nom::error::ParseError<(&[u8], usize)>, nom::error::ParseError<&[u8]>, Fn((&[u8], usize))>(nom::bits::complete::tag(0, 1_usize))(i)?;
    //let (i, _) = nom::bits::complete::tag(0x0, 1_usize)(i)?;       // should be 0 for Normal

    let p2 = nom::bits::complete::take(1_usize);
    let (i, data_or_definition): (&[u8], u8) = nom::bits::bits(p2)(i)?;

    //let p2: Fn((&[u8], usize)) -> nom::IResult<(&[u8], usize), u8> = nom::bits::complete::take(1_usize);
    //let (i, data_or_definition) = nom::bits::bits(p2)(i)?;

    //let (i, data_or_definition): (&[u8], u8) = nom::bits::bits(nom::bits::complete::take(1_usize))(i)?;
    //let (i, data_or_definition) = nom::bits::complete::take(1_usize)(i)?;      // data or definition message

    let p3 = nom::bits::complete::take(1_usize);
    let (i, developer_data_flag): (&[u8], u8) = nom::bits::bits(p3)(i)?;

    //let (i, developer_data_flag) = nom::bits::complete::take(1_usize)(i)?;    // developer data flag
    
    let p4 = nom::bits::complete::tag(0, 1_usize);
    let (i, _) = nom::bits::bits(p4)(i)?;
    
    // let (i, _) = nom::bits::complete::tag(0x0, 1_usize)(i)?;                  // reserved
    
    let p5 = nom::bits::complete::take(4_usize);
    let (i, local_mesg_num): (&[u8], u16) = nom::bits::bits(p5)(i)?;
    
    //let (i, local_mesg_num) = nom::bits::complete::take(4_usize)(i)?;         // local message type

    nom::IResult::Ok((
        i,
        FitRecordHeader::Normal(
            FitNormalRecordHeader {
                message_type: FitNormalRecordHeaderMessageType::from(data_or_definition),
                developer_fields_present: (developer_data_flag > 0),
                local_mesg_num 
            }
        )
    ))
...
/*
pub fn parse_compressed_timestamp_record_header(i: &[u8]) -> nom::IResult<&[u8], FitRecordHeader> {
    //let (i, _) = nom::bits::bits<&[u8], (&[u8], FitRecordHeader), nom::ParseError<(&[u8], usize)>, nom::ParseError<&[u8]>, >(nom::bits::complete::tag(1, 1_usize))(i)?; // 1 means Compressed
    let (i, _) = nom::bits::bits(nom::bits::complete::tag(1, 1_usize))(i)?; // 1 means Compressed

    let (i, local_mesg_num) = nom::bits::bits(nom::bits::complete::take(2_usize))(i)?;
    let (i, offset_secs) = nom::bits::bits(nom::bits::complete::take(5_usize))(i)?;
